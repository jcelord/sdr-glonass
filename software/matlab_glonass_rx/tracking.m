    ...
    if (channel(channelNr).SVN ~= 0)       
      ...
      fseek(fid, dataAdaptCoeff*...
            (settings.skipNumberOfBytes + ...
             settings.dataTypeSizeInBytes*(channel(channelNr).codePhase-1)), 'bof');
...
% Read next block of data ------------------------------------------------            
        ...
        % interation 
        rawSignal = fread(fid, dataAdaptCoeff*blksize, loopCnt_dataType);
        ...
        if (dataAdaptCoeff==2)
          ...
          if (settings_switchIQ)
              rawSignal = rawSignal2 + 1i .* rawSignal1;
          else
